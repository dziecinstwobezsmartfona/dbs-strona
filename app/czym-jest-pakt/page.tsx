    ...
    return (
        <main className="bg-(--secondary-background) min-h-screen flex flex-col items-center justify-top py-45">
            <header className="flex flex-col text-(--secondary-accent) w-3/4 lg:w-1/2 mx-auto items-center justify-center text-center">
                ...
                <p className="text-lg lg:text-xl mb-8">Rodzicielski Pakt to sposób, aby wspólnie z innymi rodzicami w swojej społeczności zobowiązać się, że nie dacie swoim dzieciom <b>smartfona przed 14-tym rokiem życia</b>, a dostępu do <b>mediów społecznościowych przed 16-tym rokiem życia</b>.</p>
                ...
                <Link className="flex justify-center bg-(--background) rounded-3xl w-full mx-auto mb-12 py-4 hover:bg-white" href="czym-jest-pakt">
                    <span className="text-lg lg:text-2xl font-title text-(--foreground)">PODPISZ PAKT! (JUŻ WKRÓTCE)</span>
                </Link>
                <p className="text-lg lg:text-xl mb-12">Podpisanie Rodzicielskiego Paktu zajmuje 30 sekund, a im więcej z nas to zrobi, tym szybciej zmienimy normy społeczne. Możliwość podpisania Rodzicielskiego Paktu już wkrótce zostanie udostępniona.</p>
                <p className="text-lg lg:text-xl mb-24">Ideą Paktu Rodzicielskiego jest ułatwienie lokalnego, wspólnego działania. <i>W tym miejscu będziemy wprowadzać aktualne dane dotyczące liczby podpisanych Paktów, dzięki czemu będzie można zobaczyć, ile rodzin w Waszej szkole i powiecie podpisało Pakt.</i></p>
